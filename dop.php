<?php

//ЕЩЕ НЕМНОГО ЦИКЛОВ И МАССИВОВ с канала Алекса Лущенко
//для понимания темы
//код нужно разкоментировать

// $a = 10;
// while ($a <= 20) {
//     echo $a . " , ";
//     $a++;
// if ($a == 15) {
//     break;

// }

// }
    
// $i = 0;
// do {
//     echo $i . " , ";
// } while (++$i <= 10);

// for($a = 10; $a; $a--){
//     echo $a . PHP_EOL;
// }

// for($a = 0; $a < 100; $a += 10){
//     echo $a . PHP_EOL;
// }

// $i = 0;
// for (; ;){
//     $i++;
//     echo $i . " ";

//     if ($i > 5) break;
// }

// for ($i = 50; $i <= 100; $i = $i + 10) {
//     echo $i . " ";
//     if($i === 80) {
//         break;
//     }
// }

// echo "\n\n";

// for ($i = 50; $i <= 100; $i = $i + 10) {
//     echo $i . " ";
//     if($i === 80) {
//         continue;
//     }
// }

// $out = '';
// for ($i = 50; $i <= 100; $i = $i + 10) {
//     $out .= $i . " ";
// }
// echo $out;

// $sum = 2;
// for ($i = 2; $i <= 5; $i++) {
//     // $sum = $sum + $i;
//     $sum += $i;
// }
// echo $sum;

// $i = 1000;
// while ($i > 0) { 
//     $i = $i - 99;
//     if ($i === 109 or $i === 703) continue;
//     echo $i . " ";
    

// }

// $s = 0;
// $i = 0;
// while ($i <= 5) {
//     $s = $s + $i; // $s += $i;
//     $i++;
// }
// echo $s;

// $studentsList = ["Adam", "John", "Bobby", "Steve", "Mikle", ];
//     foreach ($studentsList AS $key => $student) {
//         if ($key < 2) {
//         echo "$student - array $key" . " ";
//     }
//         if ($key == 2){
//             break;
//         }
//     }

//

// $arr = [2, 4, 6, 8, 10,];
//     for ($i = 0; $i < count($arr); $i++) {
//     var_dump($arr) ;
// } 
// $arr = [2 => 'dva', 4 => 'chetire', 6 => 'shest', 8 => 'vosem',];
// foreach($arr AS $key => $value) {
//     echo $key . " " . $value . PHP_EOL;
// }

// $arr1 = ['my', 'text', 1, 2, 3, 4, 5, 6, 7, 8];
// $arr2 = [rand(1, 3), rand(4, 8)];
// $sumarr = array_merge($arr1, $arr2);
// // var_dump($sumarr);
// for ($i = 0; $i <= 15; $i++) {

//     if (isset($sumarr[$i])) {
//         echo "THIS element $sumarr[$i] is exist too" . " ";

// } else {
//     echo "пук среньк" . " " . PHP_EOL;
// }

// if (in_array(3, $arr2)) {
//     echo "3 Есть внутри массива";
// } else {
//     echo "бульк";
// }

// }
